...
|--------|---------------------|------------------------|
| **Remainder Handling** | Manual cleanup needed - must check remainder and handle leftover elements with scalar code | Automatic tail handling - compiler generates cleanup code for partial vectors |
...
| **Code Complexity** | More verbose with manual index management (`global_start = tile_id * chunk_size + i * simd_width`) | Clean and declarative - focus on the operation, not indexing |

**Manual Vectorization Example:**
```mojo
# Manual cleanup needed:
var remainder = actual_tile_size % simd_width
if remainder > 0:
    # Handle leftover elements with scalar code
    
# More verbose, manual index management
for i in range(tile_size):
    global_start = tile_id * chunk_size + i * simd_width
    a_vec = a.aligned_load[simd_width](global_start, 0)
```
...
# Generates cleanup code for partial vectors

# Clean, declarative
@parameter
fn vectorized_add[width: Int](i: Int):
    # Focus on the operation, not indexing
```
